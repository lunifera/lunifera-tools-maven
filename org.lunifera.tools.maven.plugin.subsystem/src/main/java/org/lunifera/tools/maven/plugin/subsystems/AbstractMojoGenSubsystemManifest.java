...
public abstract class AbstractMojoGenSubsystemManifest extends AbstractMojo {
	 ...
	 */
	@Parameter(defaultValue = "${project.remoteArtifactRepositories}", property = "remoteRepositories", readonly = true)
	 ...
	 */
	@Parameter(defaultValue = "${project.build.outputDirectory}/OSGI-INF/SUBSYSTEM.MF", readonly = true)
	...
	private SubsystemContentHeader calculateContentFromProjectDependencies(
		...
		for (Artifact artifact : artifacts) {
			...
			if (scope.equalsIgnoreCase("Runtime")
				...
				} else if (type.equalsIgnoreCase("subsystem-feature")) {
					clause.setAttribute("type", "osgi.subsystem.feature");

				} else if (type.equalsIgnoreCase("subsystem-composite")) {
					clause.setAttribute("type", "osgi.subsystem.composite");
				} else if (type.equalsIgnoreCase("subsystem-application")) {
					clause.setAttribute("type", "osgi.subsystem.application");
				}

				if (artifact.isOptional()) {
					clause.setDirective(Clause.DIRECTIVE_RESOLUTION, "optional");
				} else if (embedContents == true) {
					try {
						...
						if (artifact.getFile().isFile()) {
							// java7 new stuff
							Files.copy(artifact.getFile().toPath(), new File(
									dependenciesDirectory, artifact.getFile()
											.getName()).toPath());
						}
					} catch (IOException e) {
						e.printStackTrace();
